...
def get_time_ranges(week_id):
    return {
        "c 10 до 12": (time(hour=10), time(hour=12)),
        "с 15 до 17": (time(hour=15), time(hour=12)),
        "с 18 до 20": "34343",
    }
...
def start(update: Update, context: CallbackContext) -> None:
    ...
    print(context.user_data['student'])
    if not context.user_data:
        update.message.reply_text('Этот чат только для студентов курсов Devman')
        return
...
def main() -> None:
    load_dotenv()
    tg_token = os.getenv("TG_USER_TOKEN")
